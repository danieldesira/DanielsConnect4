...
export default class SameDeviceGame extends Game {
  ...
  private setUpPlayerNames() {
    ...
    const onPromptOK = () => {
      ...
      const greenInput = document.getElementById(
        "dialog-input-green"
      ) as HTMLInputElement;
      if (
        redInput.value &&
      ...
      const dimensionsSelect = document.getElementById(
        "dialog-select-dimensions"
  ...
  protected landCoin(): number {
    if (this.board.getBoard()[this.currentCoinColumn][0] === Coin.Empty) {
      ...
      const coinCount = this.board.countConsecutiveCoins(
