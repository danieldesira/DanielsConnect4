...
export default class Authentication {
  ...
  public static getUserData = async () =>
    (await Authentication.authGet(
      `${config.connections.httpServer}/auth`
    )) as PlayerInfo;

  public static async loadStats() {
    Utils.enableProgressCursor();
    const stats = (await Authentication.authGet(
      `${config.connections.httpServer}/stats`
    )) as PlayerStats;
  ...
  public static initGoogleSSO(callback: Function) {
    window?.google?.accounts?.id?.initialize({
      client_id:
        "966331594657-sjtp3m7ooigjma726j7aa4kcf5qdu2v7.apps.googleusercontent.com",
  ...
  public static async updateSettings(data: PlayerSettings) {
    try {
      Utils.enableProgressCursor();
      await Authentication.authPost(
        `${config.connections.httpServer}/settings`,
        data
      );
    } catch {
      Dialog.notify({
  ...
  public static getSettings = async (): Promise<PlayerSettings> =>
    await Authentication.authGet(`${config.connections.httpServer}/settings`);
  ...
  private static async authGet(url: string): Promise<any> {
    ...
    if (auth) {
      ...
      const data =
        res.status >= 200 && res.status < 300 ? await res.json() : null;
      return data;
    } else {
      return null;
    }
